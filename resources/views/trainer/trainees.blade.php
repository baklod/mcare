...
<div class="mx-auto max-w-7xl space-y-7">
    ...
    <div class="overflow-x-auto border border-stone-200 bg-white">
        <table class="w-full min-w-[48rem] text-left text-sm"><thead class="bg-stone-50 text-xs uppercase text-stone-500"><tr><th class="px-5 py-4">Trainee</th><th class="px-5 py-4">Batch</th><th class="px-5 py-4">Schedule</th><th class="px-5 py-4">Payment</th><th class="px-5 py-4">Training state</th></tr></thead><tbody class="divide-y divide-stone-200">
        @forelse($trainees as $trainee)<tr><td class="px-5 py-4"><p class="font-bold text-stone-950">{{ $trainee->last_name }}, {{ $trainee->first_name }}</p><p class="text-stone-500">{{ $trainee->email }}</p></td><td class="px-5 py-4">{{ $trainee->batch ? $trainee->batch->name.' '.$trainee->batch->year : 'Unassigned' }}</td><td class="px-5 py-4">{{ $trainee->schedule_preference }}</td><td class="px-5 py-4">{{ $trainee->paymentStatusLabel() }}</td><td class="px-5 py-4">{{ $trainee->batch?->trainingStateLabel() ?? 'No batch' }}</td></tr>@empty<tr><td colspan="5" class="px-5 py-10 text-center text-stone-600">No approved trainees found.</td></tr>@endforelse
        </tbody></table>
    </div>
    {{ $trainees->links() }}
</div>
