...
<body class="dashboard-shell">
    ...
    <aside class="dashboard-sidebar" data-dashboard-sidebar>
        ...
        {{-- These anchors mirror the old trainer dashboard sections while keeping navigation server-rendered. --}}
        <nav class="mt-6 flex-1 overflow-y-auto pb-24 lg:pb-0">
            <div>
                <p class="dashboard-menu-label">Trainer Menu</p>
                <div class="space-y-2">
                    <a href="#dashboard" class="{{ $navItem }} is-active"><span>Dashboard</span><span class="text-xs text-white/45">Home</span></a>
                    <a href="#upload" class="{{ $navItem }}"><span>Upload Module</span><span class="text-xs text-white/45">LMS</span></a>
                    <a href="#modules" class="{{ $navItem }}"><span>My Modules</span><span class="text-xs text-white/45">Files</span></a>
                    <a href="#trainees" class="{{ $navItem }}"><span>My Trainees</span><span class="text-xs text-white/45">Class</span></a>
                    <a href="#schedule" class="{{ $navItem }}"><span>My Schedule</span><span class="text-xs text-white/45">AM / PM</span></a>
                </div>
            </div>

            <form method="POST" action="{{ route('trainer.logout') }}" class="mt-auto">
                @csrf
            ...
            <div class="flex min-h-20 flex-col gap-4 px-5 py-4 sm:flex-row sm:items-center sm:justify-between lg:px-8">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <h1 class="mt-1 font-display text-xl font-black text-slate-900">{{ $title ?? 'Trainer Portal' }}</h1>
                    </div>
                </div>
                <div class="flex flex-wrap items-center gap-2">
                    <a href="#modules" class="secondary-action py-2.5 lg:hidden">Modules</a>
                    <a href="#trainees" class="secondary-action py-2.5 lg:hidden">Trainees</a>
                    <div class="flex items-center gap-3 rounded-full border border-slate-200 bg-white py-2 pl-3 pr-4 text-sm font-bold text-slate-700 shadow-sm">
                        ...
                        <span class="hidden text-left sm:block">
                            <span class="block">{{ auth()->user()?->name ?? 'Trainer' }}</span>
    ...
    <nav class="dashboard-mobile-bar">
        ...
        <a href="#trainees" class="dashboard-mobile-link">Class</a>
