@extends('admin.layouts.app', ['title' => 'Batches | MCARE Admin'])

@section('content')
    @php
        $batch = $editingBatch;
        $formAction = $batch ? route('admin.batches.update', $batch) : route('admin.batches.store');
    ...
    <div class="space-y-6">
        <header class="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-end md:justify-between">
            <p class="text-sm text-slate-600">Create enrollment windows, assign trainers, and manage AM/PM class details.</p>
            <div class="flex flex-wrap items-center gap-2">
                <a href="{{ route('admin.training-programs.index') }}" class="inline-flex w-fit items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                    Manage programs
                </a>
        ...
        <section class="space-y-6">
            <dialog
                ...
                data-cancel-url="{{ $batch ? route('admin.batches.index') : '' }}"
            ...
            >
                <div class="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-slate-200 bg-white px-6 py-4">
                    ...
                    @if ($batch)
                        <a href="{{ route('admin.batches.index') }}" class="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-slate-200 text-slate-500 transition hover:bg-slate-50 hover:text-slate-900" aria-label="Close batch editor" title="Close">
                ...
                <form method="POST" action="{{ $formAction }}" class="mt-5 space-y-4" data-single-action data-batch-form>
                    ...
                    @if ($batch)
                        <a href="{{ route('admin.batches.index') }}" class="block text-center rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                            Cancel edit
                        </a>
                    @else
                    ...
                    <article class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                            ...
                            <div class="flex gap-2">
                                <a href="{{ route('admin.batches.edit', $item) }}" data-dashboard-prefetch class="inline-flex items-center gap-2 rounded-lg border border-purple-200 bg-white px-3 py-1.5 text-xs font-bold text-purple-700 hover:bg-purple-50">
                                ...
                                <form method="POST" action="{{ route('admin.batches.destroy', $item) }}" data-confirm="{{ $batchHasRelatedRecords ? 'This batch has related records and cannot be deleted.' : 'Delete this batch schedule? This cannot be undone.' }}">
                                    @csrf
                                    @method('DELETE')
