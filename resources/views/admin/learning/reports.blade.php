...
<section class="space-y-6">
    ...
    @forelse($batches as $batch)<tr><td><p class="font-bold text-slate-950">{{ $batch->name }} {{ $batch->year }}</p><p class="mt-1 text-xs">Deadline {{ $batch->enrollment_ends_at->format('M d, Y') }}</p></td><td><span class="dashboard-pill {{ $batch->acceptsEnrollment() ? 'bg-emerald-50 text-emerald-700 ring-emerald-100' : 'bg-slate-100 text-slate-700 ring-slate-200' }}">{{ $batch->enrollmentStateLabel() }}</span></td><td>{{ $batch->trainingStateLabel() }}</td><td class="font-bold">{{ $batch->applications_count }}</td><td>{{ $batch->am_count }}</td><td>{{ $batch->pm_count }}</td><td>{{ $batch->approved_count }}</td><td>{{ $batch->paid_count }}</td><td>{{ $batch->modules_count }}</td></tr>
